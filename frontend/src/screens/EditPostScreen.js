...
import { deletePost, editPost, listPostDetails } from '../redux/actions/postActions';
...
const EditPostScreen = ({match, location, history }) => { 
    ...
    const { success: successDelete, loading: loadingDelete, error: errorDelete } = postDelete;
	...
	}, [dispatch, userInfo,  history, post._id, post.aboutJob, post.amountSpent, post.duration, post.hirerName, post.hourlyRate, post.jobAvailability, post.jobRequirements, post.jobTitle, post.jobType, post.jobLocation, post.level, post.paymentVerification, postId, redirect, success, successDelete]);
    ...
    return ( 
		...
		<form action="" onSubmit={submitHandler} className="customer-signin-form">
			...
			<div className="customer-signin-form-group post-buttons">
                        <button type="submit" className="customer-signin-btn">Update Job Post</button>
                        
                        <button type="submit" className="delete-btn" onClick={deletePostHandler}>Delete Job Post</button>
			</div>
		</form>
	</div>
        </div>
