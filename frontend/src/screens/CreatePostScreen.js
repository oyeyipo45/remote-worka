...
const CreatePostScreen = ({ location, history }) => { 
	...
	const submitHandler = (e) => {
		e.preventDefault();
		if (jobTitle === '' || jobAvailability === '' ||aboutJob === '' || hirerName === ''  || jobLocation === ''  || jobType === ''  || jobRequirements === ''  || hourlyRate === ''  || level === ''  || paymentVerification === ''  || amountSpent === '' || duration === '') {
			setMessage('Please fill all fields');
		} else {
			dispatch(createPost({jobTitle, jobAvailability, aboutJob, hirerName, jobLocation, jobType, jobRequirements, hourlyRate, level, paymentVerification, amountSpent, duration} ));
		}
	};
    ...
    return ( 
		...
		<form action="" onSubmit={submitHandler} className="customer-signin-form">
			...
			<div className="customer-signin-form-group">
				<button type="submit" className="customer-signin-btn">Post Job</button>
			</div>
		</form>
	</div>
        </div>
